...
public class MiscUtils {
    ...
    public static <T> Set<T> parseIntRanges(String value, Function<Integer, T> map) {
            ...
            try {
                ...
                if (endpoints.length == 2) {
                    ...
                    for (int i = start; i <= end; ++i) {
                        result.add(map.apply(i));
                    }
                } else {
                    result.add(map.apply(Integer.parseInt(segment)));
                }
    ...
    public static <T> String unparseIntRanges(Set<T> value, Function<T, Integer> unmap) {
        StringBuilder result = new StringBuilder();

        Integer start = null;
        Integer last = null;

        for (T t : new TreeSet<>(value)) {
            int i = unmap.apply(t);
